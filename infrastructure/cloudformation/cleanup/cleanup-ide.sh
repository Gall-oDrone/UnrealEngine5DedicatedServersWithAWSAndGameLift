#!/bin/bash
# Delete Unreal Engine 5 IDE CloudFormation stack safely.
...
error_exit() {
    ...
    log_error "Cleanup may be incomplete. Review CloudFormation stack manually:"
    ...
    while true; do
        ...
        case "$status" in
            ...
            *)
                ...
                show_recent_events 10 "$stack_name"
                ;;
        esac
...
delete_stack() {
    ...
    case "$status" in
        DELETE_IN_PROGRESS)
            ...
            ;;
        *IN_PROGRESS)
            log_warning "Stack '$stack_name' is currently busy ($status). Waiting for it to settle before delete."
            sleep "$POLL_INTERVAL"
            ;;
    esac
...
prompt_confirmation() {
    ...
    cat <<EOF
The following CloudFormation stack will be deleted in region '$AWS_REGION':
  - $STACK_NAME
...
main() {
    ...
    log_info "=========================================="
    log_info "Unreal Engine 5 IDE Cleanup Script"
    log_info "=========================================="
    ...
    log_info "Stack '$STACK_NAME' has been deleted (or did not exist)."
